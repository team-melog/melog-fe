...
export default function AIAnalysisResult({ analysis, isLoading = false, onRetry, className = '', }) {
    ...
    const getScoreStyle = (score) => {
        if (score >= 80)
            return { color: 'text-green-700', bg: 'bg-green-50', border: 'border-green-200', emoji: '😊', label: '매우 긍정적' };
        if (score >= 60)
            return { color: 'text-blue-700', bg: 'bg-blue-50', border: 'border-blue-200', emoji: '🙂', label: '긍정적' };
        if (score >= 40)
            return { color: 'text-yellow-700', bg: 'bg-yellow-50', border: 'border-yellow-200', emoji: '😐', label: '보통' };
        if (score >= 20)
            return { color: 'text-orange-700', bg: 'bg-orange-50', border: 'border-orange-200', emoji: '😔', label: '부정적' };
