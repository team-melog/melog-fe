...
const createAudioFormData = (audioBlob: Blob, fileName = 'audio.webm'): FormData => {
...
export const convertSpeechToTextFallback = async (audioBlob: Blob): Promise<STTResponse> => {
...
// 재시도 로직이 포함된 STT 함수
export const convertSpeechToTextWithRetry = async (
  audioBlob: Blob,
  config: Partial<STTConfig> = {}
): Promise<STTResponse> => {
  const finalConfig = { ...DEFAULT_CONFIG, ...config }
